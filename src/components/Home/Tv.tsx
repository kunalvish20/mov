// src/components/Home/Tv.tsx (Example usage)
import {
  Carousel,
  CarouselContent,
  CarouselItem,
...
import { TVShowListItem } from "@/types/types"; // Import the specific list item type

const Tv = () => {
  const navigate = useNavigate();

  // Now, useFetch<TVShowListItem> is absolutely correct and matches the hook's generic usage
  const { apiList, loading, error } = useFetch<TVShowListItem>("/tv/popular");

  if (error) {
    return (
      <section className="py-10 px-4 text-center text-red-500">
            ...
            <>
              <CarouselContent>
                {apiList?.length > 0 ? (
                  ...
                  !loading && <p className="text-white px-4 py-2">No TV shows found.</p>
                )}
              </CarouselContent>
